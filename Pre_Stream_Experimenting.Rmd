...
Alright trying to add music and seeing what happens
...
penguins %>%
  dplyr::select(species, contains("mm")) %>%
  GGally::ggpairs(aes(color = species)) +
  scale_colour_manual(values = c("darkorange","purple","cyan4")) +
  scale_fill_manual(values = c("darkorange","purple","cyan4"))
...
(summary(lm(mean_bill_length ~ species, data = penguins_oh_no)))
...
penguins %>% 
  ...
  dplyr::summarise(across(
    .cols = is.numeric,
    .fns = list(mean = mean, sd = sd, median = median, min = min, max = max), na.rm = T,
    .names = "{col}_{fn}"
  )) %>% 
...
auc_penguins <- penguins_fit_rs %>% 
  ...
  roc_auc(truth = before_2009, .estimate = .pred_yes) %>% 
  print()
